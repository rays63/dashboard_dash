...
app = DjangoDash('B_chart', external_stylesheets=external_stylesheets)

# Query the CO2 data from the database
data = CONT_E.objects.all().values()
df = pd.DataFrame(list(data.values('year', 'co2','continents','co2_per_cap')))
...
def update_bubble_chart(selected_year):
    ...
    fig = px.scatter(
        ...
        x='co2_per_cap',
        y='co2',
        size='co2',  # Use 'co2' values to determine bubble size
        color='continents',
        hover_name='continents',
